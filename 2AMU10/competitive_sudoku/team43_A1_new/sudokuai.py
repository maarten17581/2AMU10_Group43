...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
        ...
        while True:
            ...
            for move in all_moves:
                ...
                eval = SudokuAI.minimax(board, game_state.taboo_moves, depth, False, 
                                        game_state.scores[0]-game_state.scores[1]+SudokuAI.pointScore[regions], alpha, beta)
                if eval > bestscore:
                    bestmove = move
                    bestscore = eval
    ...
    def minimax(board: SudokuBoard, tabooMoves: List[TabooMove], depth: int, maximizingPlayer: bool, score: int, alpha, beta):
        N = board.N
        n = board.n
        m = board.m
        ...
        for move in all_moves:
            ...
            for (k,l) in SudokuAI.box(move.i, move.j, n, m, N):
                if SudokuBoard.get(board, k, l) == SudokuBoard.empty:
                    ...
                    break
            if maximizingPlayer:
                eval = SudokuAI.minimax(board, tabooMoves, depth-1, False, score+SudokuAI.pointScore[regions], alpha, beta)
                bestscore = max(bestscore, eval)
                if bestscore >= beta:
                    break
                alpha = max(alpha, bestscore)
            else:
                eval = SudokuAI.minimax(board, tabooMoves, depth-1, True, score-SudokuAI.pointScore[regions], alpha, beta)
