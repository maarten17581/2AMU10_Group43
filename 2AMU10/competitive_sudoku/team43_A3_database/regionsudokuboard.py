...
from team43_A3_database.region import Region
from team43_A3_database.cell import Cell
...
class RegionSudokuBoard(object):
    ...
    def getCanonicalHashAfterMove(self, move):
        ...
        self.createBitBoard()
        self.bitboard[(move.value-1)][(move.i//self.n)][(move.i%self.n)][(move.j//self.m)][(move.j%self.m)] = 1
        perm = self.createCanonicalForm()
        hash = 0
        for i in range(self.N):
            for j in range(self.N):
                for k in range(self.N):
                    if self.canonicalForm[k][(i//self.n)][(i%self.n)][(j//self.m)][(j%self.m)] == 1:
    ...
    def createBitBoard(self):
        ...
        for cell in self.cells:
            if cell.value != self.emptyState:
                count += 1
                self.bitboard[(cell.value-1)][(cell.i//self.n)][(cell.i%self.n)][(cell.j//self.m)][(cell.j%self.m)] = 1
    ...
    def get_moves(self, tabooMoves: List[TabooMove]):
        ...
        possible_moves = [move for move in moves if not self.is_taboo(move[0])[0]]
        mistake_moves = [move for move in moves if move not in possible_moves]
        ...
        none_taboo_possible_moves = [move for move in possible_moves if TabooMove(move[0].i, move[0].j, move[0].value) not in tabooMoves]
        none_taboo_mistake_moves = [move for move in mistake_moves if TabooMove(move[0].i, move[0].j, move[0].value) not in tabooMoves]

        # Returns the set of moves
        return none_taboo_possible_moves, none_taboo_mistake_moves
    
    def makeMove(self, move: Move, cell: Cell, check_mistake):
        """
        ...
        wasMistake = False
        if check_mistake:
            wasMistake = self.is_taboo(move)[0]
        ...
        if check_mistake and (self.mistakeMove(cell) or wasMistake):
            return "mistake"
    ...
    def makeRandomMove(self, taboo_moves):
        ...
        madeMove = max(moves, key=lambda move: self.regionsFilledAfterMove(move))
        #cell = random.choice([cell for cell in self.cells if cell.value == self.emptyState])
        #possible_values = [index+1 for index, restrict in list(enumerate(cell.possibleValues)) if restrict == 0 and TabooMove(cell.i, cell.j, index+1) not in taboo_moves]
        #if len(possible_values) == 0:
        #    return "mistake", None, None, None
        #value = random.choice(possible_values)
        #madeMove = Move(cell.i, cell.j, value)
    ...
    def is_taboo(self, move: Move):
        solve_sudoku_path = 'bin\\solve_sudoku.exe'
        board_text = str(self)
        options = f'--move "{(move.i*self.N+move.j)} {move.value}"'
